...
export default function HomeFeedPage() {
  ...
  const t = useMemo(() => {
    const hu = { search: "Keresés...", groups: "Csoportok", posts: "Bejegyzések" };
    const en = { search: "Search...", groups: "Groups", posts: "Posts" };
  ...
  const FEED = [
    { id: "p1", authorId: "u1", authorName: "Kiss Máté", time: "2025-10-01 12:15", group: "Group 1", groupId: "grp-1", title: "TITLE-1", body: "Rövid tartalom / preview szöveg, ami több soros is lehet. Ez később csonkolható." },
    { id: "p2", authorId: "u2", authorName: "Nagy Anna", time: "2025-10-01 10:03", group: "Group 5", groupId: "grp-5", title: "TITLE-2", body: "Második bejegyzés összefoglalója. Lorem ipsum dolor sit amet, consectetur." },
    { id: "p3", authorId: "u3", authorName: "John Smith", time: "2025-09-30 18:40", group: "Group 3", groupId: "grp-3", title: "TITLE-3", body: "Third post extract. Nulla vitae elit libero, a pharetra augue." },
  ];
    ...
    <div className="min-h-screen bg-[#FFF6F2]">
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 grid gap-6 lg:grid-cols-[300px_1fr]">
        ...
        <aside className="rounded-2xl border-2 border-[#1F3351] bg-white overflow-hidden">
          <div className="bg-[#E1860E] text-white font-bold px-4 py-3">{t.groups}</div>
        ...
        <section>
          ...
          <div className="space-y-4">
            {FEED.map((p) => (
              <article key={p.id} className="rounded-2xl border-2 border-[#1F3351] bg-[#EDF5FA] shadow-sm overflow-hidden">
                <header className="px-4 py-3 border-b border-[#1F3351]/10 flex items-center gap-3">
                  <Link to={`/users/${p.authorId}`} className="w-12 h-12 rounded-full bg-white border border-[#1F3351]/30 flex items-center justify-center shrink-0">
                    <UserIcon className="w-7 h-7" />
                  </Link>
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 text-[#1F3351] font-bold truncate">
                      <Link to={`/users/${p.authorId}`} className="hover:underline truncate">{p.authorName}</Link>
                      <span className="text-[#1F3351]/60 font-medium truncate">{p.time}</span>
                      <span className="ml-auto text-[#1F3351]/80 font-semibold">
                        <Link to={`/groups/${p.groupId}`} className="hover:underline">{p.group}</Link>
                      </span>
                    </div>
                    <Link to={`/posts/${p.id}`} className="text-[#1F3351] block hover:underline font-extrabold">{p.title}</Link>
                  </div>
                </header>
                <Link to={`/posts/${p.id}`} className="block px-4 py-3 text-[#1F3351]/90">
      ...
      <button
        ...
        className="fixed bottom-6 right-6 inline-flex items-center justify-center w-14 h-14 rounded-full bg-[#E1860E] text-white text-3xl shadow-lg hover:opacity-95"
